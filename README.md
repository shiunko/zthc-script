...
### 🚀 中文脚本语言解析与执行
- **函数定义**: `函数 add(a, b) { @返回(@加(a, b)); }`（✅ 完全支持）
- **变量声明**: `变量 name: "张三";`
- **函数调用**: `@调试输出("hello", 123);` 和 `@add(10, 20);`
- **常量定义**: `常量 PI: 3.14;`
...
### 🌐 网络服务
- **TCP 服务器** (端口 7888) - 处理客户端连接和脚本执行（✅ 完全实现）
- **TCP 客户端** - 连接服务器执行远程脚本（✅ 新增）
...
```clojure
;; TCP 服务器
(require '[net.zthc.script.tcp-server :as server])

;; 启动服务器
(def test-server (server/start-server 7888))
...
;; TCP 客户端
(require '[net.zthc.script.tcp-client :as client])

;; 创建客户端连接
(def test-client (client/create-client "127.0.0.1" 7888))

;; 发送ping
(client/ping-server test-client)

;; 执行脚本
(client/execute-script test-client "变量 x: 100; @调试输出(x);")

;; 执行函数定义和调用
(client/execute-script test-client "函数 add(a, b) { @返回(@加(a, b)); }; @调试输出(@add(10, 20));")

;; 关闭客户端
(client/close-client test-client)

;; 便捷函数
(client/quick-exec "@调试输出(\"Hello TCP!\");")
```
...
├── tcp_client.clj        # TCP 客户端实现（新增）
└── util.clj              # 通用工具函数

test/net/zthc/script/     # 测试套件
├── executor_test.clj     # 执行引擎测试（包含函数定义测试）
├── transformer_test.clj  # 转换器测试
├── builtins_test.clj     # 内置函数测试
├── parser_test.clj       # 解析器测试（包含函数定义解析）
├── tcp_server_test.clj   # TCP服务器测试（新增）
├── tcp_client_test.clj   # TCP客户端测试（新增）
├── parser/
│   ├── atom_test.clj
│   ├── string_test.clj
│   ├── number_test.clj
...
**测试覆盖情况**:
- 67 个测试用例（新增函数定义和TCP功能测试）
- 385 个断言
- 覆盖所有核心模块（解析、执行、转换、内置函数、TCP服务器/客户端）
- 0 失败，0 错误
...
```
;; 定义简单函数
函数 greet(name) { @返回(@连接("你好, ", name, "!")); };

;; 定义数学函数
函数 calculate(a, b) {
    变量 sum: @加(a, b);
    变量 product: @乘(a, b);
    @调试输出("和:", sum, "积:", product);
    @返回(sum);
};
...
;; 调用函数
@调试输出(@greet("小明"));
@调试输出("计算结果:", @calculate(10, 5));
@调试输出("平均值:", @average(10, 20, 30));
```
