...
(def default-context
  "默认执行上下文"
  {:namespaces {'user builtins/builtin-functions
                'clojure.core {'+ +
                               '- -
                               '* *
                               '/ /
...
(defn create-execution-context
  ...
  (let [base-bindings (merge builtins/builtin-functions additional-bindings)]
    (sci/init {:namespaces {'user base-bindings
                            'clojure.core (get-in default-context [:namespaces 'clojure.core])}
               :classes (get default-context :classes)})))
